...
export default function OrdersStats() {
  ...
  if (stats && stats.length > 0) {
    ...
    if (obj && obj.length > 0) {
      statistics = Object.values(obj[0]);
    } else {
      console.log("No order statistics available."); // Log a graceful message
    }
  } else {
    console.log("No stats found or stats is empty."); // Log a graceful message
  }
  ...
  <p className="text-2xl font-semibold text-gray-200">
    {stats?.saleToday?.length > 0 ? `$${stats.saleToday[0]?.totalSales || "0.00"}` : "$0.00"}
  </p>
  ...
  <div className="absolute inset-x-0 bottom-0 bg-red-900 px-4 py-4 sm:px-6">
    <div className="text-sm"></div>
        ...
        <div className="relative overflow-hidden rounded-lg bg-red-800 px-4 pt-5 pb-12 shadow sm:px-6 sm:pt-6">
          ...
          <dd className="ml-16 flex items-baseline pb-6 sm:pb-7">
  {stats.orders && stats.orders[0] && stats.orders[0].minimumSale !== undefined ? (
    <p className="text-2xl font-semibold text-gray-200">
      ${stats.orders[0].minimumSale}
    </p>
  ) : (
    <p className="text-2xl font-semibold text-gray-200">Data not found</p>
  )}
  ...
  <div className="absolute inset-x-0 bottom-0 bg-pink-900 px-4 py-4 sm:px-6">
    <div className="text-sm"></div>
        ...
        <div className="relative overflow-hidden rounded-lg bg-yellow-600 px-4 pt-5 pb-12 shadow sm:px-6 sm:pt-6">
          ...
          <dd className="ml-16 flex items-baseline pb-6 sm:pb-7">
  {stats.orders && stats.orders[0] && stats.orders[0].maxSale !== undefined ? (
    <p className="text-2xl font-semibold text-gray-200">
  ...
  <p className="text-2xl font-semibold text-gray-100">
    {stats?.orders?.[0]?.totalSales !== undefined ? (
