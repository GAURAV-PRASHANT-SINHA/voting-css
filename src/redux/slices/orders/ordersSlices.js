  ...
  async (payload, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      const { data } = await axios.post(
        "http://localhost:4000/api/orders/post",
        {
          orderItems,
          shippingAddress,
          totalPrice,
...
export const fetchOrdersAction = createAsyncThunk(
  ...
  async (payload, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:4000/api/orders/getit`, config);
      return data;
    } catch (error) {
      return rejectWithValue(error?.response?.data);
    }
...
export const OrdersStatsAction = createAsyncThunk(
  ...
  async (payload, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      const { data } = await axios.get("http://localhost:4000/api/orders/sales/stats", config);
      return data;
    } catch (error) {
      return rejectWithValue(error?.response?.data);
    }
  ...
  async (productId, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      const { data } = await axios.get(
        "http://localhost:4000/api/orders/${productId}",
        config
      );
      return data;
    } catch (error) {
